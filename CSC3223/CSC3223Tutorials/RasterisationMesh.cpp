...
RasterisationMesh* RasterisationMesh::CreateLineFromPoints(const std::vector<Vector3>& inVerts,
	const std::vector<Vector4>& inColours, bool bresenham) {

	RasterisationMesh* line = new RasterisationMesh();

	if (bresenham) {
		for (int i = 0; i < (int)inVerts.size() - 1; i += 2) {
			line->RasterBresenhamLine(inVerts[i], inVerts[i + 1],
				inColours[i], inColours[i + 1]);
		}
	}
	else {
		for (int i = 0; i < (int)inVerts.size() - 1; i += 2) {
...
RasterisationMesh* RasterisationMesh::CreateTriangleFromPoints(const std::vector<Vector3>& inVerts,
	...
	if (type == 1) {
		...
		for (size_t i = 0; i < inVerts.size() - 2; i += 3) {
			tri->RasterLineEquationTriangle(inVerts[i], inVerts[i + 1], inVerts[i + 2],
				inColours[i], inColours[i + 1], inColours[i + 2]);
		}
	}
...
void RasterisationMesh::RasterBasicLine(const Vector3& p0, const Vector3& p1,
